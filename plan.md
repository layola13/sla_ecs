...
- The table-erased runtime now has verified runtime type-id metadata lookup helpers for insert/get/query/filter/changed/remove, deferred Commands, schedule access declarations, and system-param adapters. Automatic derive-style component metadata is implemented through Sla `@derive(Component)` for non-generic structs, including table/sparse-set storage metadata backed by ECS metadata `.sal` / `.sai` contracts; resource metadata is implemented through `@derive(Resource)` and consumed by `lib/resource_erased.sla`.
