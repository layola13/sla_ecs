...
- The table-erased runtime now has verified runtime type-id metadata lookup helpers for insert/get/query/filter/changed/remove, component bundle spawn/insert helpers, deferred Commands, schedule access declarations, and system-param adapters. Automatic derive-style component metadata is implemented through Sla `@derive(Component)` for non-generic structs, including table/sparse-set storage metadata backed by ECS metadata `.sal` / `.sai` contracts; resource metadata is implemented through `@derive(Resource)` and consumed by `lib/resource_erased.sla`; message metadata is implemented through `@derive(Message)` and consumed by `lib/messages_erased.sla`; event metadata is implemented through `@derive(Event)` and consumed by `lib/event_observer_erased.sla`.
- `sa_plugin_sla` remains part of the implementation surface when Sla syntax/codegen blocks ECS semantics; recent fixes include wildcard `.sla` imports, nested generic `>>` parsing, `Vec<T>` index assignment, `Vec` field method-call cleanup, generic impl protocol monomorphization for `for in`, function pointer value codegen for schedules and observers, top-level scalar const codegen, field comparison after dot access, nested indexed length expressions such as `len(world.archetypes[archetype_slot].entity_ids)`, `@derive(Component)` / `@derive(Resource)` / `@derive(Message)` / `@derive(Event)` metadata calls, `@component(storage = "SparseSet")` metadata, and expanded relative `.sai` / `.sal` import resolution.
