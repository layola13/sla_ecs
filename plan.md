...
- The table-erased runtime now has verified runtime type-id metadata lookup helpers for insert/get/query/filter/changed/remove, deferred Commands, schedule access declarations, and system-param adapters. The next runtime step is automatic derive-style component metadata generation and true multi-threaded execution over safely split World access.
