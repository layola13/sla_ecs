...
- [x] Multi-threaded mutable executor plan/thread bridge: after `lib/parallel.sla` SAB call syntax and the focused thread/function-pointer issue were fixed upstream, `lib/executor_multi_threaded.sla` now has explicit `EcsExecutorRunPlan` drive helpers plus ready-batch selection/completion. `lib/parallel_runner.sla` now bridges ready batches to singleton/pair/triple pthread-backed execution, includes `ecs_parallel_run_ready_all_up_to3`, adds access-conflict-aware nonconflicting selection via `ecs_parallel_run_ready_all_nonconflicting_up_to3`, and adds dynamic `Vec<fn>` catalogs via `ecs_parallel_run_ready_all_dynamic_catalog_up_to3`. This covers arbitrary-length catalogs with concrete dynamic waves up to width 3 and leaves conflicting ready systems for later waves; full Bevy TaskPool/Scope worker scheduling remains outside the completed behavior set. Whole-file default/SAB aggregation still has a compiler issue documented in `sa_plugin_sla/docs/sab_aggregate_mut_parallel_use_after_move_issue_cn.md`.
