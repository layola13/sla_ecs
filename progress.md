...
- [done] Verification snapshot: all current `lib/*.sla` and `examples/*.sla` files pass with installed Sla dev plugin after archetype-backed value Commands/Schedule additions; generated `.test.sa` files also pass the no-absolute-`sa_std` import check. The old `src/` prototype directory is not present in the current tree.
