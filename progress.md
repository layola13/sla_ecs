...
- [done] Verification snapshot: all current `lib/*.sla` and `examples/*.sla` files pass with installed Sla dev plugin after heterogeneous table-row storage and demo additions; generated `.test.sa` files pass the no-absolute-`sa_std` import check. The nested indexed length regression in `sa_plugin_sla/tests/test_unit_field_compare_and_nested_len.sla` also passes. The old `src/` prototype directory is not present in the current tree.
