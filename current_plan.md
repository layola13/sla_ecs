...
- Counts (measured 2026-07-07): 270 lib `.sla` modules; 174 `tests/*.sla` files; 90 `examples/*.sla`; 4,070 source `.sla` `@test` annotations across lib/tests/examples. Historical isolated-test batch total remains separately tracked in `progress.md` for older isolated batches.
- Tests verified on the SA backend; focused Batches 112–142 also pass representative default backend suites after the Batch 122 hash-map refs unblocker and the later SAB call-target/thread-function-pointer fixes, except large/import-heavy whole-file paths that are explicitly documented as compiler-size/cleanup issues. Batch 142's executor failed set-condition pending-skip helper passes generated SA and default backend with 38 isolated tests. Current ECS completion evidence should prefer generated SA (`--test-backend sa`) unless a task explicitly targets SAB behavior; SAB findings are reported to compiler docs rather than fixed directly in this stream.
...
## Completed (verified on SA backend) — see README "Bevy ECS Parity Assessment"; counts measured 2026-07-07: 270 lib / 174 tests / 4,070 source `.sla` @test total. Sub-list below is historical per-area summary
1. System Registry (8)
2. EntityCommands (14)
