...
- **映射策略**:用**项目级 derive 标记 + 普通 `impl` 方法**给每个 component/resource/message/event 产出稳定数值 id;`lib/ecs_metadata.sla`(`EcsMetadataDescriptor`)集中存布局/存储 kind/关系形状;type-erased 路径靠这些 id 做按 type-id 注册/插入/查询;`lib/reflect_runtime.sla` 还用显式 type id 消费 `AppTypeRegistry` descriptor,驱动 reflected component/resource/event/message handle 操作。
- **证据**:README ✅ "metadata IDs + verified type-id lookup helpers";`lib/ecs_metadata.sla`、`lib/world_table_erased.sla` 的 `table_erased_*_lookup` 家族,以及 `lib/reflect_runtime.sla` 的 generated-SA/default 验证。
