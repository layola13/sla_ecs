...
Measured counts: 248 lib `.sla` modules, 174 `tests/*.sla` files, 90 `examples/*.sla`, and **3,866 source `.sla` `@test` annotations** across `lib/`+`tests/`+`examples/` (3,433 in `tests/`, 354 inline in `lib/`, 79 in `examples`; historical isolated-test batch total is 3,439 after Batch 139). All listed modules cover System Registry, EntityCommands, ChangeDetection, Query completeness, Observer+Lifecycle+NonSend, Relationship traversal, ComponentInfo+EntityDisabling+BundleInfo, Schedule config, Archetype+Entity+Storage, Never, AppTypeRegistry/AppFunctionRegistry descriptor registries, explicit multi-threaded executor plan driving, ready-batch selection, ready-pair/triple bridges, width-dispatch up to 3 into the real pthread-backed runner, a looping fixed three-function ready-batch runner, access-conflict-aware ready-batch grouping, dynamic `Vec<fn>` executor catalogs that can schedule arbitrary catalog lengths through the real pthread-backed runner, a worker-count `EcsParallelTaskPool` facade that preserves requested worker counts and uses fixed width-9 arity for small batches plus a dynamic `Vec<JoinHandle<i32>>` worker path above 9, a scope-style TaskPool facade over that dynamic worker-count path, explicit scoped task sets that collect `spawn` and `spawn_on_scope` tasks and execute them through worker-count-limited pthread waves plus caller-thread local tasks, and run-plan deferred apply cleanup that records deferred systems while clearing non-deferred temporary `unapplied` state, plus library-owned recoverable Result facades and structured scene/entity remap snapshots, verified on the SA backend. Focused Batches 112–139 also pass representative default backend suites after the Batch 122 hash-map refs unblocker and the later SAB call-target/thread-function-pointer fixes, except large/import-heavy or compiler-cleanup edge paths documented under `sa_plugin_sla/docs/`; `tests/test_ecs_mut_parallel.sla` now also passes whole-file default/SAB aggregation after the `UseAfterMove tmp_67` compiler fix recorded in `sa_plugin_sla/docs/sab_aggregate_mut_parallel_use_after_move_issue_cn.md`.
...
- **Multithreaded parallel execution — partial, one wired path and one unwired path:**
  ...
  - *TaskPool facade and scoped tasks:* `EcsParallelTaskPool` now records and uses the requested worker count without clamping to the fixed arity ceiling, and exposes `ecs_parallel_task_pool_run_ready_dynamic_catalog_batch` / `ecs_parallel_task_pool_run_ready_all_dynamic_catalog` plus scope-style aliases `ecs_parallel_task_pool_scope_run_ready_dynamic_catalog_batch` / `ecs_parallel_task_pool_scope_run_ready_all_dynamic_catalog`. `EcsParallelScopedTaskSet` adds explicit `scope_spawn` / `spawn_on_scope`-style task collection and `ecs_parallel_task_pool_scope_run_tasks`, which executes threaded tasks in worker-count-limited pthread waves, executes on-scope tasks on the caller thread, and joins every threaded wave before the scope returns. This gives callers a Bevy-shaped worker-count executor entry point with verified dynamic worker counts above 9 and an explicit TaskPool scope task path over the same thread runtime.
...
- **映射策略**:**已有真线程并行**(`lib/parallel_runner.sla` 通过 `thread::spawn` 提供 pair/triple/quad/quint/sext/sept/oct/nine fixed-arity batch,并在超过 9 时用 `Vec<JoinHandle<i32>>` 动态保存 worker handles,共享 `Arc<*World>`),`lib/executor_multi_threaded.sla` 也已有显式 `EcsExecutorRunPlan` plan/drive 层来推进 ready/dependency/deferred/skip 顺序,动态 catalog 可按请求 worker-count 分发任意长度系统表,并通过 `EcsParallelTaskPool` 接收 worker-count;现有 `ecs_parallel_task_pool_scope_run_ready_*` 是 scope-style facade,`EcsParallelScopedTaskSet` 可显式收集 `spawn`/`spawn_on_scope` tasks,分别按 pool worker-count 分波执行和在 caller thread 执行,但**Bevy 的动态 TaskPool::scope 多线程执行器仍未完整复刻**——当前不是 work-stealing async task executor。
