...
├── world_table_erased.sla — Archetype table-row type-erased heterogeneous component storage with type-id metadata lookup, `Query<Entity>`, query `single`/`get`/ordered `get_many`/`iter_many` helpers, K=2..6 query combinations, query filters, binary `Or`/`And` filters, `Spawned` filters, `SpawnDetails::spawned_by` metadata, `Option`/`Has` query data, `AnyOf` query data, and optional tuple query data
