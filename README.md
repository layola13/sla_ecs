...
├── entity_set.sla    — EntitySet and EntityMap<T> with Entity value-key semantics while std hash containers remain pointer-keyed
...
- `entity_set.sla` provides `EntitySet` and `EntityMap<T>` with `Entity` value-key semantics. It currently uses `Vec<Entity>` internally because the present `sa_std` `HashMap`/`HashSet` compare key pointers rather than derived struct equality; this keeps ECS semantics correct while leaving a future internal performance swap open.
