...
├── system_param_archetype_value.sla — ArchetypeValueWorld query/resource/ResMut/message-reader system-param adapters
...
- Bevy-style dynamic query wrappers, filters, `Res<T>` / `ResMut<T>`, resource change detection, system adapters, sequential schedules, and deferred `Commands` are implemented for the current A/B world shape; the registry-owned homogeneous, type-erased, and archetype-backed value paths now also have component-id queries, commands, schedules, resources/messages, and demos. `archetype_registry.sla` verifies Bevy-style entity location migration between component-signature archetypes, `world_archetype_value.sla` connects those locations to real homogeneous component value columns and tracks resource added/changed ticks, and `system_param_archetype_value.sla` injects query, resource writer, `ResMut`, and message-reader params into system functions with adapter writeback and reader cursor advancement. Full Bevy table-row layout, automatic component metadata, and parallel execution are not complete.
