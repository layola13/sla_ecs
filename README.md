...
├── messages.sla      — Generic fixed-capacity Messages<T>, MessageWriter<T>, monotonic numeric message ids, read-with-id/get-by-id, MessageMutator-style mutable reads, Bevy-style update/update_drain/drain retention, global-id reader cursor, unread length, missed count, empty, and clear helpers
