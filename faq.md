...
- [x] **【World::insert_batch 推进】**:`lib/bundle_table_erased.sla` 已补 `table_erased_world_insert_batch_bundle2/3`,按输入顺序给已有实体插入/替换 bundle 组件。聚焦验证:`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/bundle_table_erased.sla --filter "table erased bundle insert batch updates existing entities"` 约 5.1s PASS。
- [x] **【World::insert_batch_if_new 推进】**:`lib/bundle_table_erased.sla` 已补 `table_erased_world_insert_batch_bundle2/3_if_new`,已有组件保持原值,跳过的新 erased 值会通过注册 drop_fn 释放。聚焦验证:`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/bundle_table_erased.sla --filter "table erased bundle insert batch if new preserves existing components"` 约 5.9s PASS。
...
- [x] **【只读并行查询切片已验证】**:`lib/parallel.sla` 新增基于 `thread::spawn`/`join` 的只读 shard helper;`examples/parallel_query.sla` 覆盖两条路径:① 从 `TableErasedWorld` 物化 `Query<EntityItem<Position>>`,拆成两个 `Vec<i32>` shard 后在线程中求和;② 用 `Arc<TableErasedWorld<...>>` 共享同一个 World 快照,两个线程各自执行查询并聚合范围。`lib/parallel_table_erased.sla` 进一步新增 `table_erased_readonly_parallel2`,先用 `TableErasedSystemAccess` 冲突检测拒绝相互冲突的只读系统,再把同一个 `Arc<TableErasedWorld<...>>` 快照交给两个 worker 线程。聚焦验证:`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/parallel.sla --filter "parallel i32 shard sum uses threads"` 约 2.7s PASS,`timeout 120s env SA_PLUGIN_DEV=1 sa sla test examples/parallel_query.sla --filter "parallel query demo runs table erased query shards on threads"` 约 16.6s PASS,`timeout 120s env SA_PLUGIN_DEV=1 sa sla test examples/parallel_query.sla --filter "parallel query demo reads shared table erased world snapshot on threads"` 约 16.2s PASS,`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/parallel_table_erased.sla --filter "table erased readonly parallel runner executes no conflict systems on threads"` PASS。注意:这不是并发可变 World executor;可变系统 batch 并行仍需后续基于 access graph 做安全合并/写回。
