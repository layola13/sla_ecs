...
- [x] **【DefaultQueryFilters / Allow pair system-param 推进】**:`world_table_erased` 已有 pair 与 pair-mut `Allow<T>` 查询逃逸;ordinary/observer/relationship 三条 system-param wrapper 现已补普通 pair `Query + Resource`、pair `Allow + Resource`、pair-mut `Allow` 写回 runner 及 auto type-id 版本。聚焦验证:`lib/system_param_table_erased.sla` 约 8.3s、`lib/system_param_table_erased_observer.sla` 约 10.9s、`lib/system_param_table_erased_relationship.sla` 约 12.3s 均在 `timeout 120s env SA_PLUGIN_DEV=1 sa sla test ... --filter ...` 下通过。
