...
- [x] **【DefaultQueryFilters / Allow pair system-param 推进】**:`world_table_erased` 已有 pair 与 pair-mut `Allow<T>` 查询逃逸;`system_param_table_erased` 现已补普通 pair `Query + Resource`、pair `Allow + Resource`、pair-mut `Allow` 写回 runner 及 auto type-id 版本。聚焦验证:`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/system_param_table_erased.sla --filter "table erased pair system params allow disabled entities"` 通过(约 8.3s)。后续 observer/relationship wrapper 仍需补同形 parity。
