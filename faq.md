...
- [x] **【只读并行查询切片已验证】**:`lib/parallel.sla` 新增基于 `thread::spawn`/`join` 的只读 shard helper;`examples/parallel_query.sla` 现在从 `TableErasedWorld` 物化 `Query<EntityItem<Position>>`,拆成两个 `Vec<i32>` shard 后在线程中求和。聚焦验证:`timeout 120s env SA_PLUGIN_DEV=1 sa sla test lib/parallel.sla --filter "parallel i32 shard sum uses threads"` 约 2.7s PASS,`timeout 120s env SA_PLUGIN_DEV=1 sa sla test examples/parallel_query.sla --filter "parallel query demo runs table erased query shards on threads"` 约 16.3s PASS。注意:这不是并发可变 World executor;可变系统 batch 并行仍需后续基于 access graph 做安全合并/写回。
